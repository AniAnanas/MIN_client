...
namespace Client.Data
{
    public static class CacheManager
    {
        ...
        private static readonly ConcurrentDictionary<string, object> _lastCache = new();
        ...
        public class CacheEntry<T>
        {
            ...
            {
                _key = key;
                Events = Data.CacheManager.Events[_key];
            }
...
#pragma warning restore CS8601 // Possible null reference assignment.
                ...
                return GetOrCreateCache()
                    ...
                    .Select(v =>
                    {
                        ...
                        if (v is string json)
                        {
                            ...
                            {
                                Log.Error($"Failed to deserialize JSON: {json}");
                                return default;
                            }
                        }
            ...
            public bool Init()
            {
                try
                {
                    Log.General("Initializing " + _key + " cache...");

                    TypeCache.SetType(_key, typeof(T));
                    Type t = TypeCache.GetType(_key);

                    string query = QueryCache.GetQuery(_key, "SqliteQuery");

                    if (string.IsNullOrEmpty(query))
                    {
                        Log.Error("No init query set");
                        return false;
                    }
...
#pragma warning restore CS8620 // Argument cannot be used for parameter due to differences in the nullability of reference types.
                ...
                {
                    Log.Error($"Failed to init cache {_key}: {ex.Message}");
                    return false;
                }
            }
            ...
            public bool Reload(bool save = false)
            {
                ...
                {
                    Log.Error($"Failed to reload cache: {ex.Message}");
                    return false;
                }
            }
            ...
            public bool Save()
            {
                try
                {
                    string query = QueryCache.GetQuery(_key, "SaveSqliteQuery");

                    if (string.IsNullOrEmpty(query))
                    {
                        Log.Error($"No save query set for {_key}");
                        return false;
                    }
                    ...
                    Log.General("Saving " + _key + "...");

                    var con = Database.Connection;
                    System.Data.Common.DbConnection? database = con as Microsoft.Data.Sqlite.SqliteConnection;
                    ...
                    try
                    {
                        using (var cmd = database?.CreateCommand())
                        {
                            ...
                            {
                                string aquery = QueryCache.GetQuery(_key, "SqliteQuery");
                ...
                {
                    Log.Error($"Failed to save cache {_key}: {ex}");
                    return false;
                }
            }
            ...
            private void saveToFile()
            {
                ...
                var _cacheDirectory = Path.Combine(Directory.GetCurrentDirectory(), "Cache/");
                
                string path = Path.Combine(_cacheDirectory, _key + "/");
                ...
                {
                    Log.Error($"Failed to save cache to file {filePath}: {ex.Message}");
                }
            }
        }
        ...
        public static bool SaveAll()
        {
            try
            {
                ...
                foreach (var key in _cache.Keys)
                {
                    ...
                    {
                        Log.Error($"Failed to save {key} cache: {exp.GetType().FullName}: {exp.Message}\n{exp.StackTrace}");
                        isFallback = true;
                    }
                }
                return !isFallback;
            ...
            {
                Log.Error($"Failed to save all cache: {ex.GetType().FullName}: {ex.Message}\n{ex.StackTrace}");
                return false;
            }
        }
        ...
        public static bool AutoSaveSetup(int intervalInSeconds)
        {
            StopAutoSave();
            ...
            {
                Log.Error($"Failed to auto save cache: {ex.Message}");
                return false;
            }
        }
